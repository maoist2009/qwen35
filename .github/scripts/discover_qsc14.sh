#!/usr/bin/env bash
# Fourteenth discovery: libc++.so.1 is now installed (discover13 fixed the dpkg
# lock collision), so the converter gets past the native import. discover13 then
# failed on `import yaml` (ModuleNotFoundError: No module named 'yaml'). This run
# adds the next likely Python deps (pyyaml, protobuf, flatbuffers, sympy) so we
# clear several pending missing-module errors at once, then captures --version
# and --help.
set +e
wait_apt_lock() {
  local n=0
...
mkdir -p "$WORK_DIR"
cd "$WORK_DIR"
LOG="$WORK_DIR/qsc_discover14.log"
exec > >(tee -a "$LOG") 2>&1
...
echo "libc++.so.1 location:"; find / -name 'libc++.so.1' 2>/dev/null | head -3
echo "===== setup python3.10 venv + deps (incl. yaml/protobuf/flatbuffers/sympy) ====="
...
echo "===== converter --version (py3.10 + libc++ + qti + yaml on path) ====="
"$CONV" --version 2>&1 | head -15
echo "===== converter --help (py3.10 + libc++ + qti + yaml on path) ====="
"$CONV" --help 2>&1 | head -120
echo "===== $(date -u) done ====="
