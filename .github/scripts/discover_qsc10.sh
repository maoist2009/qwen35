#!/usr/bin/env bash
# Tenth discovery: the converter needs the SDK's bundled `qti` python package
# on PYTHONPATH (numpy is the only pip dep). Locate qti, dump envsetup.sh,
# then run the LINUX qairt-converter --version / --help with qti on the path.
set +e
WORK_DIR="$HOME/litert_workspace"
mkdir -p "$WORK_DIR"
cd "$WORK_DIR"
LOG="$WORK_DIR/qsc_discover10.log"
exec > >(tee -a "$LOG") 2>&1
...
echo "CONV=$CONV"
echo "===== locate qti package + dump envsetup.sh ====="
find "$QAIRT_ROOT" -type d -name qti | head
echo "----- bin/envsetup.sh -----"
sed -n '1,80p' "$QAIRT_ROOT/bin/envsetup.sh" 2>/dev/null
echo "----- lib tree (top) -----"
ls -1 "$QAIRT_ROOT/lib" | head
echo "===== setup venv + numpy ====="
python3 -m venv "$WORK_DIR/venv"
. "$WORK_DIR/venv/bin/activate"
python -m pip install -q --upgrade pip
python -m pip install -q numpy onnx onnxruntime scipy packaging
echo "===== source envsetup + capture exact PYTHONPATH ====="
# envsetup.sh may be arch-specific; try the generic one first, then arch dir.
if [ -f "$QAIRT_ROOT/bin/envsetup.sh" ]; then
  . "$QAIRT_ROOT/bin/envsetup.sh"
fi
# Fallback: add any directory that directly contains qti/ to PYTHONPATH.
QTI_PARENTS=$(find "$QAIRT_ROOT" -type d -name qti -exec dirname {} \; | sort -u)
for p in $QTI_PARENTS; do
  case ":$PYTHONPATH:" in
    *":$p:"*) ;;
...
echo "===== converter --version (venv + qti on path) ====="
"$CONV" --version 2>&1 | head -15
echo "===== converter --help (venv + qti on path) ====="
"$CONV" --help 2>&1 | head -80
echo "===== $(date -u) done ====="
